...
- 本 playbook 只处理最终 LaTeX 凝缩稿的生成、整合和校对。
- Stage 5 的正式真源是 `artifacts/<document-slug>/final-draft.tex`。
...
- 操作：
  ...
  - 统一术语、符号和叙述口径。
  - 对引用、图表、公式等无法在当前轮次精修的部分保留清晰占位，而不是静默省略。
...
- 只有在以上问题都不构成阻塞时，才保留 `final-draft.tex` 作为正式成稿。
...
- 表现：
  - 只在聊天中给出最终稿，没有生成 `final-draft.tex`
- 处理：
  - 回到 Stage 5，先建立 LaTeX 骨架，再把内容写入运行态文件
